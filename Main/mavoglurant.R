# =============================================================================
# Main/mavoglurant.R  —  VAE-NLME for mavoglurant PK (2-cmt IV infusion)
# Dataset: nlmixr2data::mavoglurant
#
# N_eff profiles: each (ID, OCC) pair treated independently
# PK model: 2-compartment IV infusion (analytical)
#   Parameters: (CL, V1, Q, V2) in log-space
#   Dose units: mg × 1000 → μg;  DV in ng/mL;  V in L;  CL in L/h
...
source(file.path(root_dir, "R", "functions_mavo.R"))
...
# =============================================================================
# 1. Load data  (occasion-split)
# =============================================================================
loaded        <- load_data_mavo()
data          <- loaded$data
data_in       <- loaded$data_in
lengths       <- loaded$lengths
rate          <- loaded$rate      # [N_eff] μg/h
t_inf         <- loaded$t_inf     # [N_eff] h
...
# 2. Dimensions
# =============================================================================
nbatch <- data$shape[1]   # N_eff  ≈ 198
x_dim  <- 2L              # (TAD, conc)
z_dim  <- 4L              # (CL, V1, Q, V2)
n_cov  <- covariates$shape[2]   # 4

# =============================================================================
# 3. Prior / link
# AMT=25mg × 1000 = 25000 μg; DV_max≈1730 ng/mL;  V1 ≈ 25000/1730 ≈ 14.5 L
# Use: CL=25 L/h, V1=15 L, Q=20 L/h, V2=150 L
# =============================================================================
h         <- function(x) torch_exp(x)
h_inverse <- function(x) torch_log(x)

h_dim  <- 50L
sigma0 <- torch_log(torch_tensor(c(0.1, 0.1, 0.1, 0.2)))
mu0    <- torch_tensor(c(25.0, 15.0, 20.0, 150.0))

Encoder <- lstm_encoder(x_dim, h_dim, z_dim, n_cov, mu0, sigma0, h_inverse)

# =============================================================================
# 4. Decoder  (analytical 2-cmt IV infusion)
# =============================================================================
Decoder <- function(z_normal, time, h) {
  decoder_mavo_2cmt(z_normal, time, h, rate, t_inf)
}

# =============================================================================
# 5. Covariate design matrix
# =============================================================================
C_list         <- initalize_C_mavo(nbatch, z_dim, n_cov, covariates)
C              <- C_list$C
C_regression   <- C_list$C_regression
names_co <- c(
  "AGE_CL",  "SEX_CL",  "WT_CL",  "HT_CL",
  "AGE_V1",  "SEX_V1",  "WT_V1",  "HT_V1",
  "AGE_Q",   "SEX_Q",   "WT_Q",   "HT_Q",
  "AGE_V2",  "SEX_V2",  "WT_V2",  "HT_V2"
)
...
for (i in seq_len(nbatch)) {
  phi_opt[i,] <- EmpiricalBayesEstimate_mavo(
...
printOutput_generic(z_pop, omega_pop, a, b, z_dim, nbatch,
                    lengths$sum(), h,
                    param_names  = c("CL_pop", "V1_pop", "Q_pop", "V2_pop"),
                    omega_names  = c("omega_CL", "omega_V1", "omega_Q", "omega_V2"),
                    names_co     = names_co,
                    LL_lin_mu    = LL_lin_mu,
                    LL_is        = LL_is)
...
), save_dir = file.path(root_dir, "Results"), dataset_name = "mavoglurant")
...
plotConvergence_pop_generic(Elbo_iter, a_iter, z_pop_iter, omega_pop_iter,
                             ...
                             save_path = "Plots/mavo_convergence_popParam.pdf")
...
plotConvergence_covariate_generic(z_pop_iter, z_dim, iters, kl_iter, gamma_iter,
                                   ...
                                   save_path = "Plots/mavo_convergence_covariate.pdf")

# =============================================================================
# VPC + PPC
# =============================================================================
plot_vpc_mavo(data, lengths, z_pop, omega_pop, a, b, C, z_dim, rate, t_inf,
              n_sim=500L, nbins=10,
              save_path=file.path(root_dir,"Plots","mavoglurant_vpc.pdf"))
plot_ppc_mavo(data, lengths, Encoder, data_in, covariates_in,
              a, b, rate, t_inf, z_dim,
              n_sim=200L, nbins=10,
              save_path=file.path(root_dir,"Plots","mavoglurant_ppc.pdf"))
